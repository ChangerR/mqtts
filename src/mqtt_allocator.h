...
class MQTTAllocator {
public:
    MQTTAllocator(const std::string& id, const std::string& tag, size_t limit = 0, MQTTAllocator* parent = nullptr);
    ...
    MQTTAllocator* create_child(const std::string& child_id, const std::string& tag, size_t limit = 0);
...
#define MQ_MEM_FREE(client_id, ptr, size) if (ptr) MQ_MEM_MANAGER.get_allocator(client_id)->deallocate(ptr, size)
