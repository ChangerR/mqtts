...
struct SharedMessageContent
{
  ...
  // 增加引用计数
  void add_ref() const { ref_count.fetch_add(1); }
...
struct PendingMessageInfo
{
  ...
  // 构造函数 - 从消息基本信息创建
  PendingMessageInfo(const MQTTString& topic, const MQTTByteVector& payload, uint8_t qos,
                     bool retain, bool dup, const Properties& properties,
                     const MQTTString& sender_id, const MQTTString& target_id)
...
struct PendingMessage : public PendingMessageInfo
{
  ...
  PendingMessage(const PublishPacket& packet, const MQTTString& target, const MQTTString& sender)
      : PendingMessageInfo(packet.topic_name, packet.payload, packet.qos, packet.retain, packet.dup,
                           packet.properties, sender, target)
  {
  }

  // 从PendingMessageInfo创建
  PendingMessage(const PendingMessageInfo& info) : PendingMessageInfo(info) {}

  // 兼容性接口 - 这些方法现在委托给基类
  const MQTTString& get_sender_client_id() const
  {
    return PendingMessageInfo::get_sender_client_id();
  }
};
...
class MessageContentCache
{
 ...
 public:
  ...
  SharedMessageContentPtr get_or_create_content(const MQTTString& topic,
                                                const MQTTByteVector& payload, uint8_t qos,
                                                bool retain, bool dup, const Properties& properties,
                                                const MQTTString& sender_id)
  {
    ...
    // 创建新内容
    SharedMessageContentPtr new_content(
   ...
   */
  SharedMessageContentPtr get_or_create_content_from_packet(const PublishPacket& packet,
                                                            const MQTTString& sender_id)
  {
    return get_or_create_content(packet.topic_name, packet.payload, packet.qos, packet.retain,
                                 packet.dup, packet.properties, sender_id);
  }
  ...
  void cleanup_expired_content(int max_age_seconds = 300)
  {
    ...
    while (it != content_cache_.end()) {
      if (!it->second.is_valid() || (now - it->second->timestamp) > max_age) {
