...
MQTTAllocator::MQTTAllocator(const std::string& id, const std::string& tag, size_t limit, MQTTAllocator* parent)
    : id_(id), tag_(tag), limit_(limit), parent_(parent), usage_(0) {}

MQTTAllocator::~MQTTAllocator() {
    for (std::unordered_map<std::string, MQTTAllocator*>::iterator it = children_.begin(); it != children_.end(); ++it) {
        delete it->second;
    }
    children_.clear();
}
...
void* MQTTAllocator::allocate(size_t size) {
    ...
    if (limit_ > 0 && usage_ + size > limit_) {
        LOG->warn("Memory allocation failed: limit exceeded for id {} tag {} (limit: {}, used: {}, requested: {})",
    ...
    if (ptr) {
        ...
        LOG->debug("Allocated {} bytes for id {} tag {} (used: {}/{})",
                  size, id_.c_str(), tag_.c_str(), usage_.load(), limit_);
    }
    return ptr;
}
...
void MQTTAllocator::deallocate(void* ptr, size_t size) {
    ...
    LOG->debug("Freed {} bytes from id {} tag {} (used: {}/{})",
              size, id_.c_str(), tag_.c_str(), usage_.load(), limit_);
}

MQTTAllocator* MQTTAllocator::create_child(const std::string& child_id, const std::string& tag, size_t limit) {
    if (children_.count(child_id)) return nullptr;
...
size_t MQTTAllocator::get_total_memory_usage() const {
    ...
    for (std::unordered_map<std::string, MQTTAllocator*>::const_iterator it = children_.begin(); it != children_.end(); ++it) {
