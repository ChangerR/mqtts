 ...
 */
struct MemoryConfig {
  size_t initial_pool_size = 64 * 1024 * 1024;   // 初始内存池大小 (64MB)
  size_t max_pool_size = 512 * 1024 * 1024;      // 最大内存池大小 (512MB)
  size_t block_size = 4096;                      // 内存块大小
  bool use_tcmalloc = true;                      // 是否使用tcmalloc
  size_t buffer_initial_size = 1024;            // 缓冲区初始大小
  size_t buffer_max_size = 1024 * 1024;         // 缓冲区最大大小
};
