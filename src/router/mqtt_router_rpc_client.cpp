...
int MQTTRouterRpcClient::send_request(MessageType type, const MQTTByteVector& payload,
                                      MQTTByteVector& response) {
    int ret = -1;
    ...
    if (!connected_) {
        ...
        if (send_message(message) != 0) {
            ...
            if (!completed) {
                ...
                ret = pending_request->error_code;
                if (ret == 0) {
