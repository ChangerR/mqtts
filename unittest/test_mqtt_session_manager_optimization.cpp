...
void test_shared_content_ref_count(TestFramework& test)
{
  ...
  {
    // 创建共享内容
    SharedMessageContent* content =
        new SharedMessageContent(topic, payload, qos, retain, dup, properties, sender_id);
    ...
    // 验证引用计数正确性
    test.assert_true(
        initial_ref_count && ref_count_after_ptr1 && ref_count_after_copy && ref_count_after_reset,
        "引用计数管理不正确");
...
void test_pending_message_info(TestFramework& test)
{
  ...
  PendingMessageInfo message_info(topic, payload, qos, retain, dup, properties, sender_id,
                                  target_id);
  ...
  test.assert_true(topic_correct && payload_correct && qos_correct && retain_correct &&
                       dup_correct && sender_correct && target_correct,
                   "消息信息访问不正确");
}
...
void test_pending_message_compatibility(TestFramework& test)
{
  ...
  test.assert_true(
      target_correct && sender_correct && valid && topic_correct && payload_correct && qos_correct,
...
void test_message_content_cache(TestFramework& test)
{
  ...
  // 获取共享内容
  SharedMessageContentPtr content1 =
      cache.get_or_create_content(topic1, payload1, qos, retain, dup, properties, sender_id);
  SharedMessageContentPtr content2 =
      cache.get_or_create_content(topic2, payload2, qos, retain, dup, properties, sender_id);

  // 再次获取相同的内容
  SharedMessageContentPtr content1_again =
      cache.get_or_create_content(topic1, payload1, qos, retain, dup, properties, sender_id);
  ...
  test.assert_true(cache_working && different_content && cache_size_correct, "消息内容缓存不正确");
}
...
void test_performance_comparison(TestFramework& test)
{
  ...
  // 测试优化方式的内存使用
  SharedMessageContentPtr shared_content(new SharedMessageContent(
      large_packet.topic_name, large_packet.payload, large_packet.qos, large_packet.retain,
  ...
  // 计算内存使用估算
  size_t traditional_memory =
      traditional_messages.size() * (sizeof(PendingMessage) + large_packet.payload.size());
  ...
  double memory_savings =
      (double)(traditional_memory - optimized_memory) / traditional_memory * 100;
