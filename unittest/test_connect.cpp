...
int main()
{
  ...
  if (ret == 0 && packet != nullptr) {
    ...
    if (packet->type == mqtt::PacketType::CONNECT) {
      ...
      // 详细输出每个属性
      if (connect->properties.session_expiry_interval != 0)
        std::cout << "session_expiry_interval: " << connect->properties.session_expiry_interval
                  << std::endl;
      if (connect->properties.receive_maximum != 0)
        std::cout << "receive_maximum: " << connect->properties.receive_maximum << std::endl;
      if (connect->properties.maximum_packet_size != 0)
        std::cout << "maximum_packet_size: " << connect->properties.maximum_packet_size
                  << std::endl;
      if (connect->properties.topic_alias_maximum != 0)
        std::cout << "topic_alias_maximum: " << connect->properties.topic_alias_maximum
                  << std::endl;
      if (connect->properties.topic_alias != 0)
      ...
      if (!connect->properties.user_properties.empty()) {
        ...
        for (size_t i = 0; i < connect->properties.user_properties.size(); ++i) {
          std::cout << "[" << connect->properties.user_properties[i].first << ": "
                    << connect->properties.user_properties[i].second << "] ";
        }
        std::cout << std::endl;
      }
      if (!connect->properties.authentication_method.empty())
        std::cout << "authentication_method: " << connect->properties.authentication_method
                  << std::endl;
      if (!connect->properties.authentication_data.empty())
        std::cout << "authentication_data: (size=" << connect->properties.authentication_data.size()
                  << ")" << std::endl;
      if (!connect->properties.assigned_client_identifier.empty())
        std::cout << "assigned_client_identifier: "
                  << connect->properties.assigned_client_identifier << std::endl;
      if (connect->properties.server_keep_alive != 0)
        std::cout << "server_keep_alive: " << connect->properties.server_keep_alive << std::endl;
      if (!connect->properties.response_information.empty())
        std::cout << "response_information: " << connect->properties.response_information
                  << std::endl;
      if (!connect->properties.server_reference.empty())
      ...
      if (connect->properties.payload_format_indicator != 0)
        std::cout << "payload_format_indicator: "
                  << (int)connect->properties.payload_format_indicator << std::endl;
      if (connect->properties.message_expiry_interval != 0)
        std::cout << "message_expiry_interval: " << connect->properties.message_expiry_interval
                  << std::endl;
      if (!connect->properties.content_type.empty())
      ...
      if (!connect->properties.correlation_data.empty())
        std::cout << "correlation_data: (size=" << connect->properties.correlation_data.size()
                  << ")" << std::endl;
      if (connect->properties.subscription_identifier != 0)
        std::cout << "subscription_identifier: " << connect->properties.subscription_identifier
                  << std::endl;
      if (connect->properties.will_delay_interval != 0)
        std::cout << "will_delay_interval: " << connect->properties.will_delay_interval
                  << std::endl;
      if (connect->properties.maximum_qos != 0)
